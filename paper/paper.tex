...
residue emulation, implemented in the open library \fpemu{}: a
double-precision matrix multiplication that runs on those integer units
through the residue arithmetic of the Ozaki-II scheme: operands are scaled
to exact integers, multiplied modulo a set of pairwise-coprime moduli, one
INT8 GEMM per modulus, and the exact product is recovered by the Chinese
Remainder Theorem. On top of this scheme we contribute a runtime
Cauchy--Schwarz sizing pass that measures the operands and converts unused
residue range into operand precision, so that every modulus count is safe
and adversarial input narrows the result instead of corrupting it; an
extraction pass that runs entirely in integer arithmetic on the raw
floating-point encoding, with round-to-nearest splitting and a first-order
quantization correction carried by two additional INT8 GEMMs; a
reconstruction that computes balanced mixed-radix digits with packed
byte-dot-product instructions entirely in registers; and a fused
tensor-core engine whose epilogue reduces every product to a one-byte
balanced residue, dispatched against cuBLAS by measured per-shape timing
rather than architectural guesswork. Measured on five NVIDIA architectures
spanning Ampere to Blackwell, \fpemu{} exceeds native FP64 by up to a
factor of 26 on the parts whose FP64 units are throttled, and at equal
GEMM counts it exceeds \gemmul{}, the reference implementation published
by the scheme's authors, at every size on every such part, by factors
between 1.07 and 3.80; on datacenter silicon with unthrottled FP64,
emulation of either flavor pays only at the largest sizes, and there
\gemmul{} leads. A tiled Cholesky factorization that routes its trailing
updates through the batched entry point runs 1.5 times faster end to end
at equal residuals. Accuracy exceeds native DGEMM in every distribution
tested, and on exactly representable inputs the product is reproduced bit
for bit, a property verified against integer oracles at depths to
$K = 200{,}000$. The implementation, including batched and transpose-free
entry points, is publicly available.
\end{abstract}
...
Blackwell workstation parts widen this ratio further. The hardware trend is
not accidental. Deep learning workloads, which dominate the economics of
GPU design, need low-precision matrix engines, and double-precision units
survive at full rate only on a shrinking set of datacenter parts. For the
large community computing on everything else, double-precision matrix
...
Figure~\ref{fig:pipeline} shows the five phases of \fpemu{}. This section
...
$A$ and $B$ is exactly representable at its row or column scale, the
residuals vanish, the reconstructed integer is the true product, and the
output is bit-identical to exact arithmetic whenever each inner product
fits the range; our test suite verifies this by \code{torch.equal} against
integer oracles, including random integer matrices, the aligned all-ones
case, a dyadic case with per-column scales spanning $2^{-12}$ to $2^{0}$,
and, through the segmented path of Section~\ref{sec:extensions}, depths to
$K = 200{,}000$. On general inputs the exact path returns the correctly
rounded double of the reconstructed corrected product, whose distance from
the true real product is set by the second-order term $r_A r_B$ alone.
\end{remark}
...
along exactly that line. The two locally held RTX parts were measured
under repeated best-of discipline; the Blackwell, A100, and H200 numbers
come from cloud instances. Throughput is reported as $2n^3$ over the
median of timed executions after warm-up; native FP64 is
\code{torch.matmul} on the same tensors. The first call at a new problem
...
arithmetic. The accuracy tables use $M = N = 128$; a spot check at
$512 \times 4096 \times 512$ reproduces the pattern (native 49.4 bits,
default 53.2, reduced 44.6).
...
FP64-throttled architectures. Ratios are \fpemu{} over \gemmul{}.}
\label{tab:gemmul8}
\small
...
The default exceeds native FP64 DGEMM in every cell.}
\label{tab:accuracy}
\begin{tabular}{lrrrr}
\toprule
distribution & $K$ & native & default & reduced \\
\midrule
