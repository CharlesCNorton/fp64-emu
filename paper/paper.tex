...
are narrowed instead. The consequence is a scheme in which every
configuration the library plans is safe. Adversarial input costs correct
bits; it cannot corrupt the answer.
...
make the ceiling $256$ rather than $128$, since for even $m$ the
balanced representatives lie in $[-m/2,\, m/2 - 1]$, which at $m = 256$
is exactly the signed eight-bit range $[-128, 127]$.
...
rule would demand: for dense Gaussian rows the norm-product bound scales
as $2b + \log_2 K - \log_2(2 \ln K)$, about $2b + 8$ at $K = 4096$
against the worst-case $2b + \log_2 K$, and for rows dominated by a few
large entries the saving approaches the full $\log_2 K$;
Section~\ref{sec:ablation} quantifies what the pass delivers. Second, and
more important for robustness, $g$ may be negative: if the caller
provisions fewer moduli than the data requires, the operands are narrowed
until the bound fits. Narrowing is bounded below by the width floor
$b_e \ge 16$ discussed with Proposition~\ref{prop:range}; every modulus
count above that floor's envelope, including every count the planner
emits, is range-safe, and the count becomes a pure
accuracy-versus-speed knob. \gemmul{}'s fast
...
\]
with $c_b$ chosen to cancel the bias inside the sum. Each limb product
stays below $2^{26}$ and the limb sum below $2^{27}$, so the Barrett
quotient underestimates the true quotient by at most one and a single
conditional subtraction completes the reduction; no 64-bit multiply-high
appears anywhere in the pass. The integer formulation is also exact where an
...
\]
computed in the balanced representative range $[-m_t/2,\, m_t/2)$, fit
signed bytes, and every factor $M_u \bmod m_t$ can be
...
and the groups combine high to low with a single wide multiply each, in
the three-group form $P = a_0 + M_{0..6}\,(a_1 + M_{7..13}\, a_2)$. At
the default fourteen moduli two groups suffice, so one wide multiply
replaces thirteen 128-bit steps; counts above fourteen engage the third
group and a second multiply. A register-residency
...
The implementation floors $b_e$ at $16$, since the correction scale
$2^{b_e - 15}$ must remain a positive shift, and does not re-check the
margin once the floor binds. The hypothesis therefore has to be
satisfiable at the floor: a 16-bit worst-case product needs
$33 + \lceil \log_2 K \rceil$ bits of range, which seven moduli provide
at every single-launch depth and eight at the segmented maximum, and the
planner never emits fewer than thirteen. Only a hand-set count below
that envelope can drive the floor into violation; there the hypothesis
fails and the reconstruction can wrap.
...
\[
  |V_{ij} - S_{ij}|
  \;\le\; K \Bigl( 3 \cdot 2^{b_e - 7} + \tfrac92 \cdot 2^{b_e - 15}
  + \tfrac14 \Bigr),
\]
...
the correction is worth a factor of $128/3$, about $5.4$ bits, in the
worst case.
\end{proposition}
...
respectively, totaling $3 \cdot 2^{b_e-8} + \tfrac94 \cdot 2^{b_e-15}$
for one pair; the symmetric pair contributes the same, doubling both
constants, and the uncorrected second-order term adds
$|r^A r^B| \le \tfrac14$. Summing over $K$ terms gives the stated bound.
Without the correction the first-order term alone contributes up to
$2 \cdot 2^{b_e - 1} = 2^{b_e}$ per term.
\end{proof}

The bound explains the measured behavior: the corrected worst case sits
about five and a half bits below the uncorrected ceiling, and the four
to five bits observed on random data (Section~\ref{sec:accuracy})
realize most of that margin.
...
$0.56$ seconds. The non-update remainder differs slightly between the
configurations, $0.20$ against $0.25$ seconds on Ada, reflecting
allocator and cache state left by the different update engines rather
than any difference in the shared code. The factorization residuals
$\|A - LL^{\top}\|_F / \|A\|_F$ are equal between the two engines at
$3$ to $4 \times 10^{-16}$.

\subsection{Energy}\label{sec:energy}

Throughput converts into energy through board power, which we sample by
repeated \code{nvidia-smi} queries throughout a sustained loop on the two
locally held parts (Table~\ref{tab:energy}). The per-product times
reproduce Table~\ref{tab:native} to within two percent, so the two
harnesses measure the same quantity.

\begin{table}[t]
\centering
\caption{Energy per product at $n = 8192$: mean board power over a
sustained loop, and the resulting joules per GEMM.}
\label{tab:energy}
\begin{tabular}{llrrrr}
\toprule
part & engine & ms & W & J & GFLOP/J \\
\midrule
RTX 6000 Ada & native & 840.0 & 180.5 & 151.6 & 7.3 \\
 & default & 83.2 & 297.3 & 24.7 & 44.5 \\
consumer Ampere & native & 3907.4 & 77.9 & 304.5 & 3.6 \\
 & default & 250.8 & 114.5 & 28.7 & 38.3 \\
\bottomrule
\end{tabular}
\end{table}

The emulation draws more power on both parts, 65 percent more on Ada and
47 percent more on consumer Ampere, and finishes 10 to 16 times sooner,
so it delivers $6.1\times$ and $10.6\times$ more work per joule
respectively. The direction of the power gap is itself a statement about
the hardware: a throttled FP64 pipe cannot occupy the machine, leaving
the board drawing well under its envelope, while the residue pipeline
runs the tensor cores at full draw and still wins on energy by finishing
