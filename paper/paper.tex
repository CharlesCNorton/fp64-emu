...
Board power sampled by \code{nvidia-smi} during sustained loops converts
the throughput advantage into an energy one. On Ada at $n = 8192$,
native FP64 draws 180 W for 840 ms per product, 151.6 J per GEMM, while
the default configuration draws 297 W for 83 ms, 24.7 J per GEMM: the
emulation pulls 65 percent more power and finishes ten times sooner, so
it is $6.1\times$ more energy-efficient, 44.5 against 7.3 GFLOP per
joule. The asymmetry in draw is itself informative: the throttled FP64
pipe cannot fill the machine and leaves the board underdrawn, while the
emulation runs the tensor cores at the full power envelope and still
wins on energy by finishing an order of magnitude sooner.
